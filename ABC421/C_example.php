...
// 文字列中の全ての'B'の位置を配列に記録
// 例: "AABBBA" の場合、$pos = [2, 3, 4] となる
for($i=0;$i<$n*2;$i++){
    if($s[$i]=="B") {
        $pos[] = $i;
    }
...
// 2つのパターンでの最小操作回数を計算
$ans1 = 0; // パターン1: ABABAB... (偶数位置にA、奇数位置にB)
$ans2 = 0; // パターン2: BABABA... (偶数位置にB、奇数位置にA)

// 各位置での移動コストを計算
for($i=0;$i<$n*2;$i++){
    if($i%2==0) {
        // 偶数位置: パターン1では'A'が来るべき位置
        // $i>>1 は $i/2 の意味（ビットシフト演算）
        // $pos[$i>>1] は i/2番目の'B'の現在位置
        $ans1 += abs($pos[$i>>1]-$i);
    }
    if($i%2==1) {
        // 奇数位置: パターン2では'A'が来るべき位置
        $ans2 += abs($pos[$i>>1]-$i);
    }
}

// 2つのパターンのうち操作回数が少ない方を出力
echo min($ans1,$ans2);
